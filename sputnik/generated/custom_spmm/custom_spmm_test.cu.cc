...
class CustomSpmmTest : public ::testing::Test {
 public:
   ...
   */
  void CustomSpmm(int m, int k, int n, const float *a_values,
...
TYPED_TEST_SUITE(CustomSpmmTest, TestProblems);
...
template <typename Problem>
void TestFn(FloatCustomSpmmFn custom_spmm_fn, CustomSpmmTest<Problem> *obj) {
  ...
  // Run the gpu kernel.
  CUDA_CALL(custom_spmm_fn(obj->kDimM, obj->kDimK, obj->kDimN,
  ...
  obj->CustomSpmm(obj->kDimM, obj->kDimK, obj->kDimN, sparse_matrix.Values(),
...
template <typename Problem>
void HalfTestFn(HalfCustomSpmmFn custom_spmm_fn, CustomSpmmTest<Problem> *obj) {
...
#define REGISTER_TESTCASE(name, fn) \
  TYPED_TEST(CustomSpmmTest, name) { TestFn(fn, this); }

#define REGISTER_FLOAT_TESTCASE_HELPER(name, tname, fn, stype, dtype, mt, kt, \
                                       nt, bs)                                \
  const auto &tname = fn<CustomSpmmConfig<float, stype, dtype, mt, kt, nt, bs>>;    \
...
#define REGISTER_FLOAT_TESTCASE(name, fn, stype, dtype, mt, kt, nt, bs)    \
  REGISTER_FLOAT_TESTCASE_HELPER(name, ANONYMOUS_NAME(custom_spmm_fn), fn, stype, \
                                 dtype, mt, kt, nt, bs)

/* 1-d tiling with blocksize 64 */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float, 1, 32, 64, 32);

/* 2-d tiling with blocksize 64 and vector loads */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float2, 2, 32, 64, 16);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float4, 4, 32, 64, 8);

/* 1-d tilings with blocksize 32 */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float, 1, 32, 32, 32);

/* 2-d tilings with 32 n-dim and vector loads */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float2, 2, 32, 32, 16);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float4, 4, 32, 32, 8);

REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float4, 4, 16, 32, 8);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float4, 4, 8, 32, 8);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float2, 2, 16, 32, 16);

REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float4, 2, 32, 64, 16);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float4, 1, 32, 128, 32);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float, float2, 1, 32, 64, 32);

REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float4, 2, 64, 64, 16);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float4, 1, 64, 128, 32);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float2, 1, 64, 64, 32);

/* 2-d tilings with 16 n-dim and vector loads */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float2, 4, 32, 16, 8);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float, 2, 32, 16, 16);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float4, 8, 32, 16, 4);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float2, 4, 32, 16, 8);

/* 2-d tilings with 8 n-dim and vector loads */
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float2, float2, 8, 32, 8, 4);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float4, 16, 32, 8, 2);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float2, 8, 32, 8, 4);
REGISTER_FLOAT_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, float4, float, 4, 32, 8, 8);
...
#define REGISTER_TESTCASE(name, fn) \
  TYPED_TEST(CustomSpmmTest, name) { HalfTestFn(fn, this); }

#define REGISTER_HALF_TESTCASE_HELPER(name, tname, fn, stype, dtype, mt, kt, \
                                      nt, bs)                                \
  const auto &tname = fn<CustomSpmmConfig<half2, stype, dtype, mt, kt, nt, bs>>;   \
  REGISTER_TESTCASE(name##_##stype##x##dtype##x##mt##x##kt##x##nt##x##bs, tname)

#define REGISTER_HALF_TESTCASE(name, fn, stype, dtype, mt, kt, nt, bs)    \
  REGISTER_HALF_TESTCASE_HELPER(name, ANONYMOUS_NAME(custom_spmm_fn), fn, stype, \
                                dtype, mt, kt, nt, bs)

/* 1-d tiling with blocksize 64 */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half2, half2, 1, 32, 64, 32);

/* 2-d tiling with blocksize 64 and vector loads */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half4, half4, 2, 32, 64, 16);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half8, 4, 32, 64, 8);

/* 1-d tilings with blocksize 32 */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half2, half2, 1, 32, 32, 32);

// /* 2-d tilings with 32 n-dim and vector loads */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half4, half4, 2, 32, 32, 16);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half8, 4, 32, 32, 8);

/* 2-d tilings with 16 n-dim and vector loads */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half4, half4, 4, 32, 16, 8);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half4, half2, 2, 32, 16, 16);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half8, 8, 32, 16, 4);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half4, 4, 32, 16, 8);

/* 2-d tilings with 8 n-dim and vector loads */
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half4, half4, 8, 32, 8, 4);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half8, 16, 32, 8, 2);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half4, 8, 32, 8, 4);
REGISTER_HALF_TESTCASE(CudaCustomSpmmEx, CudaCustomSpmmEx, half8, half2, 4, 32, 8, 8);
