...
#include "sputnik/generated/custom_spmm/compute_utils.h"
#include "sputnik/generated/custom_spmm/cuda_custom_spmm.h"
#include "sputnik/generated/custom_spmm/dense_tile.h"
#include "sputnik/generated/custom_spmm/output_tile.h"
#include "sputnik/generated/custom_spmm/predicate_utils.h"
#include "sputnik/generated/custom_spmm/sparse_tile.h"
#include "sputnik/generated/custom_spmm/custom_spmm_config.h"
...
struct CustomSpmmKernel {
  ...
  static __device__ __forceinline__ void KernelFn(
    ...
    //
    /// Begin custom_spmm residue computation.
    //
...
__global__ void __launch_bounds__(Config::kThreadsPerBlock)
  ...
  CustomSpmmKernel<Config>::KernelFn(m, k, n, row_indices, values, row_offsets,
...
using FloatTable = std::unordered_map<std::string, FloatCustomSpmmFn>;
...
FloatTable* GetFloatTable() {
  static FloatTable kernel_table = {
      ...
      {MakeHandle(920, 920, 196, -1),
       CudaCustomSpmmEx<CustomSpmmConfig<float, float, float4, 1, 32, 128, 32>>},
      {MakeHandle(920, 464, 196, -1),
       CudaCustomSpmmEx<CustomSpmmConfig<float, float, float4, 1, 32, 128, 32>>},
      {MakeHandle(232, 115, 3136, -1),
       CudaCustomSpmmEx<CustomSpmmConfig<float, float, float4, 4, 8, 32, 8, 4, false>>},
      {MakeHandle(232, 232, 3136, -1),
       CudaCustomSpmmEx<CustomSpmmConfig<float, float2, float4, 4, 16, 32, 8, 4, false>>},
      // MBV1 W1.7
...
FloatCustomSpmmFn GetKernel(int m, int k, int n, int nonzeros) {
...
cudaError_t CudaCustomSpmmBiasRelu(
  ...
  auto custom_spmm_kernel = GetKernel(m, k, n, nonzeros);
  if (custom_spmm_kernel) {
    return custom_spmm_kernel(m, k, n, nonzeros, row_indices, values, row_offsets,
  ...
  if ((n % 4) == 0) {
    if (n == 8) {
      ...
      typedef CustomSpmmConfig<float, float4, float, 4, 32, 8, 8, 4, false> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    } else if (n < 8) {
      typedef CustomSpmmConfig<float, float4, float, 4, 32, 8, 8> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    } else if (n == 16) {
      // No predicates in the n-dimension.
      typedef CustomSpmmConfig<float, float4, float2, 4, 32, 16, 8, 4, false> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    } else if (n < 16) {
      typedef CustomSpmmConfig<float, float4, float2, 4, 32, 16, 8> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    } else if (n == 32) {
      // No predicates in the n-dimension.
      typedef CustomSpmmConfig<float, float4, float4, 4, 32, 32, 8, 4, false> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    } else if ((n % 64) == 0) {
      ...
      typedef CustomSpmmConfig<float, float4, float4, 4, 32, 64, 8, 4, false, true, 8>
          Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
      ...
      typedef CustomSpmmConfig<float, float4, float4, 4, 32, 32, 8> Config;
      return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                                row_offsets, column_indices, dense_matrix, bias,
                                output_matrix, stream);
    }
  } else if ((n % 2) == 0) {
    typedef CustomSpmmConfig<float, float2, float2, 2, 32, 32, 16> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else {
    // Scalar kernel.
    typedef CustomSpmmConfig<float, float, float, 1, 32, 32, 32> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  }
...
cudaError_t CudaCustomSpmm(int m, int k, int n, int nonzeros,
                     ...
                     float* __restrict__ output_matrix, cudaStream_t stream) {
  return CudaCustomSpmmBiasRelu(m, k, n, nonzeros, row_indices, values, row_offsets,
...
cudaError_t CudaCustomSpmmBiasRelu(
  ...
  if (n < 16) {
    typedef CustomSpmmConfig<half2, half8, half2, 4, 32, 8, 8, 4> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else if (n == 16) {
    typedef CustomSpmmConfig<half2, half8, half2, 4, 32, 8, 8, 4, false> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else if (n < 32) {
    typedef CustomSpmmConfig<half2, half8, half4, 4, 32, 16, 8, 4> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else if (n == 32) {
    typedef CustomSpmmConfig<half2, half8, half4, 4, 32, 16, 8, 4, false> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else if (n > 32 && ((n % 64) == 0)) {
    typedef CustomSpmmConfig<half2, half8, half8, 4, 32, 32, 8, 4, false> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else {
    typedef CustomSpmmConfig<half2, half4, half4, 2, 32, 32, 16, 4> Config;
    return CudaCustomSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  }
...
#define INSTANTIATE_TILED_FLOAT(fn, stype, dtype, mt, kt, nt, bs)           \
  template cudaError_t fn<CustomSpmmConfig<float, stype, dtype, mt, kt, nt, bs>>( \
...
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float, 1, 32, 64, 32);

/* 2-d tiling with blocksize 64 and vector loads */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float2, 2, 32, 64, 16);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float4, 4, 32, 64, 8);

/* 1-d tiling with blocksize 32 */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float, 1, 32, 32, 32);

/* 2-d tilings with 32 n-dim and vector loads */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float2, 2, 32, 32, 16);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float4, 4, 32, 32, 8);

/* Hybrid kernels for small problems */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float4, 4, 16, 32, 8);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float4, 4, 8, 32, 8);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float2, 2, 16, 32, 16);

/* Vector kernels without subwarp tiling */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float4, 2, 32, 64, 16);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float4, 1, 32, 128, 32);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float, float2, 1, 32, 64, 32);

/* Big vector kernels without subwarp tiling */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float4, 2, 64, 64, 16);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float4, 1, 64, 128, 32);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float2, 1, 64, 64, 32);

/* 2-d tilings with 16 n-dim and vector loads */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float2, 4, 32, 16, 8);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float, 2, 32, 16, 16);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float4, 8, 32, 16, 4);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float2, 4, 32, 16, 8);

/* 2-d tilings with 8 n-dim and vector loads */
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float2, float2, 8, 32, 8, 4);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float4, 16, 32, 8, 2);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float2, 8, 32, 8, 4);
INSTANTIATE_TILED_FLOAT(CudaCustomSpmmEx, float4, float, 4, 32, 8, 8);
#endif  // SPUTNIK_BUILD_TEST

#undef INSTANTIATE_TILED_FLOAT

#define INSTANTIATE_TILED_HALF(fn, stype, dtype, mt, kt, nt, bs)               \
  template cudaError_t fn<CustomSpmmConfig<half2, stype, dtype, mt, kt, nt, bs>>(    \
      int, int, int, int, const int*, const half2*, const int*, const short2*, \
      const half2*, const float*, half2*, cudaStream_t);

#ifdef SPUTNIK_BUILD_TEST
/* 1-d tiling with blocksize 64 */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half2, half2, 1, 32, 64, 32);

/* 2-d tiling with blocksize 64 and vector loads */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half4, half4, 2, 32, 64, 16);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half8, 4, 32, 64, 8);

/* 1-d tiling with blocksize 32 */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half2, half2, 1, 32, 32, 32);

/* 2-d tilings with 32 n-dim and vector loads */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half4, half4, 2, 32, 32, 16);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half8, 4, 32, 32, 8);

/* 2-d tilings with 16 n-dim and vector loads */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half4, half4, 4, 32, 16, 8);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half4, half2, 2, 32, 16, 16);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half8, 8, 32, 16, 4);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half4, 4, 32, 16, 8);

/* 2-d tilings with 8 n-dim and vector loads */
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half4, half4, 8, 32, 8, 4);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half8, 16, 32, 8, 2);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half4, 8, 32, 8, 4);
INSTANTIATE_TILED_HALF(CudaCustomSpmmEx, half8, half2, 4, 32, 8, 8);
#endif  // SPUTNIK_BUILD_TEST
