...
__global__ void __launch_bounds__(kBlockItemsY* kBlockWidth)
    CudaCustomSddmmKernel(int m, int k, int n, const int* __restrict__ row_indices,
...
cudaError_t CudaCustomSddmm(int m, int k, int n, int nonzeros,
...
cudaError_t CudaCustomSddmmEx(
  ...
  CudaCustomSddmmKernel<LoadType, kBlockItemsY, kBlockItemsK, kBlockItemsX,
...
INSTANTIATE_TILED(CudaCustomSddmmEx, float, 1, 32, 32, 32);
INSTANTIATE_TILED(CudaCustomSddmmEx, float2, 2, 32, 32, 16);
INSTANTIATE_TILED(CudaCustomSddmmEx, float4, 4, 32, 32, 8);
