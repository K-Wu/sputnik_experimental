...
class CustomSddmmTest : public ::testing::Test {
 public:
   ...
   */
  void CustomSddmm(int m, int k, int n, const int* row_offsets,
...
TYPED_TEST_SUITE(CustomSddmmTest, TestProblems);
...
template <typename Problem>
void TestFn(CustomSddmmFn custom_sddmm_fn, CustomSddmmTest<Problem>* obj) {
  ...
  obj->CustomSddmm(obj->kDimM_, obj->kDimK_, obj->kDimN_, output_matrix.RowOffsets(),
...
#define REGISTER_TESTCASE(name, fn) \
  TYPED_TEST(CustomSddmmTest, name) { TestFn(fn, this); }
...
#define REGISTER_TILED_TESTCASE(name, fn, ltype, mt, kt, nt, bs)            \
  REGISTER_TILED_TESTCASE_HELPER(name, ANONYMOUS_NAME(custom_sddmm_fn), fn, ltype, \
                                 mt, kt, nt, bs)

REGISTER_TILED_TESTCASE(CudaCustomSddmmEx, CudaCustomSddmmEx, float, 1, 32, 32, 32);
REGISTER_TILED_TESTCASE(CudaCustomSddmmEx, CudaCustomSddmmEx, float2, 2, 32, 32, 16);
REGISTER_TILED_TESTCASE(CudaCustomSddmmEx, CudaCustomSddmmEx, float4, 4, 32, 32, 8);
